...
if __name__ == "__main__":
    # Simulamos el DataFrame que produce data_fetcher.py (Medellín)
    import sys, os
    sys.path.insert(0, os.path.dirname(__file__))
    from data_fetcher import fetch_solar_data

    print("=== Obteniendo datos solares ===")
    df_solar = fetch_solar_data(6.2442, -75.5812)

    if df_solar is None:
        print("[FALLO] No se obtuvieron datos de NASA POWER.")
        sys.exit(1)

    print("\n=== Calculando energía mensual ===")
    df_energia = calcular_energia_mensual(df_solar)

    cols_show = ["mes", "radiacion_kwh", "energia_dia_kwh", "energia_mes_kwh", "dias_mes"]
    print(df_energia[cols_show].to_string(index=False))

    print("\n=== KPIs del sistema (4 kWp residencial) ===")
    kpis = calcular_kpis(df_energia)
    separadores = {
        "potencia_pico_kwp":    "--- TÉCNICOS ---",
        "ahorro_anual_cop":     "\n--- FINANCIEROS ---",
        "co2_evitado_kg_anual": "\n--- AMBIENTAL ---",
    }
    for k, v in kpis.items():
        if k in separadores:
            print(separadores[k])
        unidad = {
            "potencia_pico_kwp":    "kWp",
            "energia_anual_kwh":    "kWh/año",
            "energia_diaria_media": "kWh/día",
            "mejor_mes_kwh":        "kWh",
            "peor_mes_kwh":         "kWh",
            "ahorro_anual_cop":     "COP",
            "ahorro_mensual_medio": "COP",
            "payback_anios":        "años",
            "ahorro_vida_util_cop": "COP",
            "roi_pct":              "%",
            "co2_evitado_kg_anual": "kg CO₂/año",
        }.get(k, "")
        print(f"  {k:28s}: {v:>12} {unidad}")